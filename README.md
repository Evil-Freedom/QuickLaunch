...
| 触发方式 | 说明 | 实现 |
| --- | --- | --- |
| 定时 | 每天 / 工作日 / 周末 / 一次性 / 自定义星期 | `AlarmManager` 精确闹钟，触发后自动重排下一次 |

> 自定义星期：在「重复」里选「自定义」后勾选星期几（可多选），仅在这些星期触发，与固定时刻或随机窗口均可叠加。

> 跳过中国法定节假日：建规则时勾选「跳过中国法定节假日（含调休休息日）不触发」，该定时规则在法定节假日当天不触发。节假日数据由 App 启动时自动同步（也可在主界面点「同步法定节假日」手动刷新），离线也能用。未同步到数据时该选项不生效（规则照常触发）。

> 多数据源同步：节假日支持多个数据源（timor.tech、holiday-cn 的 GitHub 与 jsDelivr 两份镜像）。同步时**依次尝试、第一个成功即采用**；主界面「数据源」下拉可指定优先源或选「自动（推荐）」（自动模式会记忆上次成功的源并下次优先）。任一源失败自动回退其余源，全部失败才提示网络异常，不会因单点故障导致功能失效。

> 手动管理节假日：主界面点「手动管理节假日」可**离线**自行增删休息日（含调休），不依赖任何网络源。适合所有数据源都不可达时的兜底，或补录尚未被数据源收录的本地特殊休息日。增删后所有定时规则立即按新数据重新排程。

> 自定义数据源：主界面点「管理数据源」可手动添加自己的节假日 API（填名称、URL 模板含 `{year}`、选择解析格式 timor.tech / holiday-cn），保存后即接入多源回退链，与内置源一起参与同步尝试；内置源不可删，自定义源可随时删除。
